...
// ============ Constants ============
const STATS_CARDS = [
  { title: "Pedalangan", key: "total_pedalangan", gradient: "from-emerald-500 to-teal-600" },
  { title: "Padangsari", key: "total_padangsari", gradient: "from-teal-500 to-cyan-600" },
  { title: "Total Keseluruhan", key: "total_keseluruhan", gradient: "from-emerald-600 to-green-700" },
] as const;

const KELURAHAN = {
  PEDALANGAN: "Pedalangan",
  PADANGSARI: "Padangsari",
} as const;
...
// ============ Utility Functions ============
function normalizeUser(user: any): UserLocation {
  return {
    ...user,
...
function filterUsersByKelurahan(users: UserLocation[], kelurahan: string, searchQuery: string) {
  const query = searchQuery.trim().toLowerCase();
  return users.filter(
    (u) =>
      u.kelurahan?.toLowerCase() === kelurahan.toLowerCase() &&
      (u.nama?.toLowerCase().includes(query) || u.email?.toLowerCase().includes(query))
  );
}

function getCountColor(count: number) {
  if (count > 10) return "bg-gradient-to-r from-emerald-600 to-teal-700 text-white";
  if (count > 5) return "bg-gradient-to-r from-emerald-500 to-teal-600 text-white";
  return "bg-gradient-to-r from-emerald-400 to-teal-500 text-white";
}

// ============ Sub Components ============
function StatsCard({ title, count, gradient }: StatsCardProps) {
  return (
    <div
...
function SearchUserCard({ user, onClick }: { user: UserLocation; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
    >
      <div className="flex items-start gap-4 p-5 rounded-3xl bg-white border-2 border-emerald-300 shadow-sm hover:shadow-lg hover:-translate-y-0.5 hover:border-emerald-500 transition-all">
        ...
        <div className="min-w-0">
          ...
          {addr && <p className="text-gray-500 text-sm mt-0.5 line-clamp-1">{addr}</p>}
        </div>
      </div>
    </button>
  );
...
function RWCard({ rw, count, onClick, delay }: RWCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="group/item relative p-5 rounded-2xl bg-gradient-to-br from-white to-emerald-50/30 border border-emerald-200/60 transition-all duration-300 cursor-pointer animate-fade-in hover:border-emerald-400 hover:shadow-lg hover:ring-1 hover:ring-emerald-300/60"
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-emerald-400/0 via-emerald-400/10 to-emerald-400/0 -translate-x-full group-hover/item:translate-x-0 transition-transform duration-700" />
      ...
      <div className="relative flex items-center justify-between mb-3">
        ...
        {hasUsers ? (
          <div className="flex items-center gap-2">
            ...
            <span className={`px-3 py-1 rounded-full text-sm font-bold ${getCountColor(count)}`}>
              {count} orang
            </span>
          </div>
        ) : (
  ...
  return (
    <Card className="group relative overflow-hidden border-0 shadow-2xl rounded-3xl backdrop-blur-xl bg-white/90 hover:shadow-emerald-300/50 transition-all duration-700 hover:scale-[1.01]">
      ...
      <CardHeader className="relative flex items-center gap-4 border-b border-emerald-100 bg-gradient-to-r from-emerald-50/80 to-teal-50/80 py-5 px-6">
        <div className="relative">
          <div className={`absolute inset-0 bg-gradient-to-r ${gradient} rounded-xl blur-md opacity-50 group-hover:opacity-100 transition-opacity duration-500`} />
      ...
      <CardContent className="p-6 space-y-4 relative">
        {isSearching ? (
          searchResults.length > 0 ? (
            ...
            <div className="p-5 rounded-3xl bg-emerald-50 border-2 border-emerald-200 text-emerald-900 shadow-sm">
    ...
    <div className="flex items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1 sm:mt-0.5">Distribusi kasus Diabetes Melitus di Banyumanik</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full">
      <div className="relative max-w-4xl mx-0">
        <div className="group relative rounded-2xl ring-1 ring-emerald-200/70 border-2 border-emerald-200/40 bg-white shadow-sm hover:shadow-md transition-all focus-within:ring-emerald-500 focus-within:border-emerald-500">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-emerald-500/80" />
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Cari nama pengguna"
            className="w-full h-14 rounded-2xl bg-transparent pl-12 pr-12 outline-none placeholder:text-gray-400 text-gray-800"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-2 top-1/2 -translate-y-1/2 h-9 w-9 grid place-items-center rounded-xl hover:bg-emerald-50 active:scale-95 transition text-emerald-600"
...
function MapCard({ users, onUserClick }: { users: UserLocation[]; onUserClick: (u: any) => void }) {
  return (
    <Card className="group relative overflow-visible border-0 shadow-2xl rounded-3xl backdrop-blur-xl bg-white/80 hover:shadow-emerald-200/50 transition-all duration-700 hover:scale-[1.01]">
      ...
      <CardHeader className="relative flex items-center justify-between gap-4 border-b border-emerald-100 bg-gradient-to-r from-white/80 to-emerald-50/50 py-5 px-6 backdrop-blur-sm">
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-r from-emerald-400 to-teal-500 rounded-xl blur-md opacity-50 group-hover:opacity-100 transition-opacity duration-500" />
      ...
      <CardContent className="p-6">
        <div className="w-full h-[500px] rounded-2xl border-2 border-emerald-100 overflow-visible">
          <div className="h-full w-full rounded-2xl overflow-hidden">
            <InteractiveMap users={users} onUserClick={onUserClick} />
          </div>
        </div>
      </CardContent>
    </Card>
...
export default function LokasiPersebaran() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (usersRes.data.success) {
        const normalizedUsers = (usersRes.data.data || []).map(normalizeUser);
  ...
  const resultsByKelurahan = {
    Pedalangan: filterUsersByKelurahan(users, KELURAHAN.PEDALANGAN, searchLower),
    Padangsari: filterUsersByKelurahan(users, KELURAHAN.PADANGSARI, searchLower),
  };
  ...
  return (
    <div className="min-h-screen bg-white px-6 md:px-10 py-9">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* Map Card */}
        <MapCard users={users} onUserClick={handleUserClick} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <RWSection
            kelurahan={KELURAHAN.PEDALANGAN}
            rwData={statistics?.pedalangan_rw || []}
            onRWClick={handleRWCardClick}
            gradient="from-emerald-500 to-teal-600"
            searchQuery={search}
            searchResults={resultsByKelurahan.Pedalangan}
            onUserClick={handleUserClick}
          />
          <RWSection
            kelurahan={KELURAHAN.PADANGSARI}
            rwData={statistics?.padangsari_rw || []}
            onRWClick={handleRWCardClick}
            gradient="from-teal-500 to-cyan-600"
            searchQuery={search}
            searchResults={resultsByKelurahan.Padangsari}
            onUserClick={handleUserClick}
          />
        </div>
      </div>
