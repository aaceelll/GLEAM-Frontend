          ...
          <div className="text-xs space-y-1">
            ...
            {!review.is_correct && (
              <p>
                ...
                <span className="text-gray-900">{review.correct_answer_text}</span>
                <span className="ml-2 text-gray-600">(Skor: {review.correct_score})</span>
              </p>
            )}
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-4xl rounded-2xl bg-white border-2 border-gray-100 shadow-2xl overflow-hidden">
        <div className="max-h-[85svh] overflow-y-auto">
          <ModalHeader
            icon={ClipboardList}
            title={`Review Jawaban - ${d.type === "post" ? "Post Test" : "Pre Test"}`}
            subtitle={`${d.nama} - ${d.bank_name}`}
            onClose={onClose}
            testId="close-review"
          />
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="rounded-2xl border-2 border-gray-100">
              ...
              <div className="p-5 space-y-4 max-h-96 overflow-y-auto">
                {review.length > 0 ? (
                  review.map((r, idx) => <ReviewItem key={r.soal_id} review={r} idx={idx} />)
                ) : (
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-3xl rounded-2xl bg-white border-2 border-gray-100 shadow-2xl overflow-hidden">
        <div className="max-h-[80svh] sm:max-h-[85svh] overflow-y-auto">
          <ModalHeader
            icon={ClipboardList}
            title={`Riwayat ${type === "post" ? "Post Test" : "Pre Test"}`}
            subtitle={userName}
            onClose={onClose}
                ...
                {history.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${getScoreColor(item.score)}`}>
                        {item.score}%
                      </span>
                      <button
  ...
  return (
    <div className="min-h-screen bg-white px-6 md:px-10 py-9">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1 sm:mt-0.5">Riwayat Pre Test & Post Test</p>
            </div>
          </div>
        </div>
                    ...
                    filtered.map((row, idx) => (
                      ...
                      >
                        ...
                        <td className="py-4 text-center">
                          <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${getScoreColor(row.latestScore)}`}>
                            {row.latestScore}%
                          </span>
                        </td>
        ...
        <ReviewModal open={reviewOpen} onClose={() => setReviewOpen(false)} data={reviewData} />
      </div>
    </div>
  );
}