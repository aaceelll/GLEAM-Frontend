  ...
  return (
    <div
      className={`${HOVER_CARD} ${
        active ? "border-emerald-500 ring-1 ring-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50 shadow-md" : ""
      }`}
      onClick={onSelect}
      role="button"
      tabIndex={0}>
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div
              className={`w-11 h-11 rounded-xl grid place-items-center shadow ${
                active ? "bg-gradient-to-br from-emerald-500 to-teal-500" : "bg-emerald-50"
              }`}>
          ...
          <div className="flex-1 min-w-0 w-full">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                ...
                {soal.tipe === "screening" ? "Screening" : "Pilihan ganda"}
              </span>
            </div>
    ...
    <div className="min-h-screen bg-white px-6 md:px-10 py-9">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <section className="bg-white/80 backdrop-blur-sm rounded-3xl border-2 border-gray-100 shadow-xl overflow-hidden">
          <SectionHeader title="Bank Soal" subtitle="Kumpulan kategori/kelompok soal" count={banks.length} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {filteredBanks.map((bank) => (
                  <BankCard
                    ...
                    onRename={() => setOpenRename({ id: bank.id, nama: bank.nama })}
                    onDelete={() => setConfirmDeleteBank({ id: bank.id, nama: bank.nama })}
                  />
                ))}
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                {filteredSoal.length === 0 ? (
                  <EmptyState
                    ...
                    description={soal.length === 0 ? 'Klik tombol "Tambah Soal" untuk membuat pertanyaan pertama' : "Tidak ditemukan hasil pencarian"}
                  />
                ) : (
                  <div className="space-y-4">
      ...
      <ConfirmModal
        ...
        description={
          <>
            Apakah Anda yakin ingin menghapus <span className="font-semibold">{confirmDeleteBank?.nama}</span> beserta seluruh soalnya? Tindakan ini tidak dapat
            dibatalkan.
          </>
        }
        confirmText="Hapus"
