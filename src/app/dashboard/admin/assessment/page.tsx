...
const HOVER_CARD =
  "group relative overflow-hidden rounded-2xl border-2 border-gray-100 bg-white " +
...
// ============ Utility Functions ============
function parseErrText(err: any, fallback: string): string {
...
// Helpers
const utils = {
  normalize: (s: string) => s.replace(/^(pre-test|post-test)\s*/i, "").trim().toLowerCase(),
  isPre: (s: string) => /^pre-test/i.test(s),
  isPost: (s: string) => /^post-test/i.test(s),
  newest: (arr: BankSoal[]) =>
    arr.sort((a, b) =>
...
// Build pairs
function buildPairs(banks: BankSoal[]) {
  // group by normalized subject
  const groups = new Map<string, BankSoal[]>();
  banks.forEach((b) => {
    const key = utils.normalize(b.nama);
    (groups.get(key) ?? groups.set(key, []).get(key)!)?.push(b);
  });

  // convert to array & sort by newest update
  return Array.from(groups.values())
    .sort((a, b) =>
      new Date(utils.newest(b)?.updatedAt ?? 0).getTime() -
      new Date(utils.newest(a)?.updatedAt ?? 0).getTime()
    )
    .map((list) => {
      const pre = utils.newest(list.filter((x) => utils.isPre(x.nama)));
      const post = utils.newest(list.filter((x) => utils.isPost(x.nama)));
      const subject = list[0].nama.replace(/^(pre-test|post-test)\s*/i, "").trim();

      return { pre, post, subject };
    });
}
    ...
    <div className="flex items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        <div className="relative isolate shrink-0">
          <span aria-hidden className="absolute -inset-1.5 sm:-inset-2 rounded-2xl bg-gradient-to-br from-emerald-400/25 to-teal-500/25 blur-lg -z-10"/>
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1 sm:mt-0.5">Kelola bank soal & kuisioner skoring di satu halaman</p>
        </div>
      </div>
    </div>
  );
...
function FlashMessage({ msg }: { msg: Msg }) {
  ...
  return (
    <div
      ...
      {msg.type === "success" ? <CheckCircle2 className="h-6" /> : <AlertCircle className="h-6" />}
...
// Bank Card
function BankCard({ bank, active, onSelect, onRename, onDelete, note, fullWidth,
}: {
  bank: BankSoal;
  active: boolean;
  onSelect: () => void;
  ...
  return (
    <div className={`${fullWidth ? "md:col-span-2" : ""}`}>
      <div
        className={`${HOVER_CARD} ${active ? "border-emerald-500 ring-2 ring-emerald-400 bg-emerald-50" : ""}`}
        onClick={onSelect}
        role="button"
        tabIndex={0}
      >
        ...
        <div className="relative p-5">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3 min-w-0">
              <div className={`w-11 h-11 rounded-xl grid place-items-center shadow ${active ? "bg-gradient-to-br from-emerald-500 to-teal-500" : "bg-emerald-50"}`}>
                <FolderOpen className={`h-6 w-6 ${active ? "text-white" : "text-emerald-600"}`} />
              </div>
              <div className="min-w-0">
                <h3 className="font-bold text-gray-900 leading-snug line-clamp-2">{bank.nama}</h3>
                <div className="text-xs text-gray-500 flex items-center gap-2 flex-wrap">
                  <span>{bank.totalSoal} soal</span>
                  {bank.totalSoal > 0 ? (
                    <>
                      <span className="text-gray-300">•</span>
                      <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-semibold">✓ Ditampilkan ke user</span>
                    </>
                  ) : (
                    <>
                      <span className="text-gray-300">•</span>
                      <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">Belum ada soal</span>
                    </>
                  )}
                </div>
                {note ? <p className="mt-2 text-xs text-amber-700 font-semibold">{note}</p> : null}
              </div>
            </div>
            <div className="flex items-center gap-2 z-10 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
              <button onClick={onRename} className="p-2 rounded-lg text-amber-600 bg-amber-50 hover:bg-amber-500 hover:text-white transition-colors shadow-sm" title="Ubah Nama" aria-label="Ubah Nama">
                <Pencil className="h-4 w-4" />
              </button>
              <button onClick={onDelete} className="p-2 rounded-lg text-red-600 bg-red-50 hover:bg-red-500 hover:text-white transition-colors shadow-sm" title="Hapus" aria-label="Hapus">
...
// Soal Card
function SoalCard({ soal, index, onDelete }: { soal: Soal; index: number; onDelete: () => void }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border-2 border-gray-100 bg-white p-4 sm:p-5 hover:border-emerald-300 hover:shadow-lg transition-all">
        ...
        <div className="flex items-start gap-3 w-full">
          ...
          <div className="flex-1 min-w-0 w-full">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="font-semibold text-gray-900 leading-snug break-words">{soal.teks}</h3>
              <span className="px-2 py-0.5 rounded-full text-xs font-semibold border whitespace-nowrap bg-emerald-50 text-emerald-700 border-emerald-200">
                Pilihan ganda
              </span>
            </div>
            {soal.opsi && soal.opsi.length > 0 && (
              <div className="mt-3 flex flex-col gap-2 w-full">
                {soal.opsi.map((opt) => (
                  <div key={opt.no} className="flex items-start gap-2 px-3 py-2 rounded-xl w-full border bg-gray-50 border-gray-200">
...
/* ===================== GreenModalFrame / ConfirmModal / Loading / Empty / SectionHeader ===================== */
function GreenModalFrame({ titleIcon, title, subtitle, onClose, children, maxWidth = "max-w-xl",
}: {
...
function ConfirmModal({ open, title, description, confirmText = "Hapus", onConfirm, onClose,
}: {
  open: boolean;
  title: string;
    ...
    <div className="fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white border-2 border-gray-100 shadow-2xl">
        <div className="px-5 py-4 border-b-2 border-gray-100 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 transition" aria-label="Close">
...
function LoadingSpinner({ text = "Memuat..." }: { text?: string }) {
  return (
    <div className="py-10 flex flex-col items-center justify-center">
      ...
      <p className="text-sm text-gray-500 mt-4 font-medium">{text}</p>
    </div>
  );
}
...
function SectionHeader({ title, subtitle, count, actionButton,
}: {
  title: string;
  subtitle: string;
  count?: number;
    ...
    <div className="px-6 py-5 border-b-2 border-gray-100 bg-gradient-to-r from-emerald-50 to-teal-50">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1 ml-4">{subtitle}</p>
        </div>
        {count !== undefined ? (
...
export default function AssessmentPage() {
  ...
  const [options, setOptions] = useState<{ label: string; skor: string }[]>(INITIAL_OPTIONS);
  ...
  const fetchBanks = useCallback(async () => {
    ...
    try {
      ...
      // set banks directly (we will derive pairs in render)
      setBanks(mapped);
      if (!selectedBankId && mapped.length) {
        // pick the newest group/entry as default selection
        const newest = mapped
          .slice()
          .sort((a, b) => {
        ...
        if (newest) setSelectedBankId(newest.id);
      }
    } catch {
  ...
  const fetchSoal = useCallback(async (bankId: string | null) => {
    ...
    try {
      ...
      const mapped: Soal[] = list.map((it: any) => ({
        ...
        tipe: "pilihan_ganda",
        opsi: (it.opsi ?? []).map((o: any, idx: number) => ({
            no: idx + 1,
            teks: o.teks,
            skor: o.skor,
          })) ?? [],
  ...
  // load soal when selected bank changes
  useEffect(() => {
    fetchSoal(selectedBankId);
  }, [selectedBankId]);

  // Handlers untuk CRUD bank soal
  async function handleCreateBank() {
  const name = addBankName.trim();
  const valid = /^(Pre-Test|Post-Test)\s+.+$/i;

  if (!name)
    return showMsg({ type: "error", text: "Nama bank soal tidak boleh kosong." });

  if (!valid.test(name)) {
    setOpenAddBank(false);
    return showMsg({
      type: "error",
      text: "Gunakan format: Pre-Test ... atau Post-Test ...",
    });
  }

  try {
    await api.post(API_PATHS.bankList, { nama: name });
  ...
  // derive pairs from (filtered) banks
  const bankPairs = useMemo(() => buildPairs(filteredBanks), [filteredBanks]);
    ...
    <div className="min-h-screen bg-white px-6 md:px-10 py-9">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <section className="bg-white/80 backdrop-blur-sm rounded-3xl border-2 border-gray-100 shadow-xl overflow-hidden">
          <SectionHeader title="Bank Soal" subtitle="Kumpulan Bank Soal" count={banks.length} />

          <div className="px-6 py-4">
            <SearchInput value={bankQuery} onChange={setBankQuery} placeholder="Cari bank soal..." />
          </div>

          <div className="px-4 pb-6">
            {banksLoading ? (
              <LoadingSpinner text="Memuat bank soal..." />
            ) : filteredBanks.length === 0 ? (
              <EmptyState icon={FolderOpen} title="Belum Ada Bank Soal" description="Klik tombol di atas untuk membuat bank soal" />
            ) : (
              <div className="space-y-5">
                {/* Kalau cuma 1 test bank yang muncul -> single wide card */}
                {bankPairs.map((pair, idx) => {
                  const pre = pair.pre;
                  const post = pair.post;
                  return (
                    <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      {pre ? (
                        <BankCard
                          bank={pre}
                          active={selectedBankId === pre.id}
                          onSelect={() => setSelectedBankId(pre.id)}
                          onRename={() => setOpenRename({ id: pre.id, nama: pre.nama })}
                          onDelete={() => setConfirmDeleteBank({ id: pre.id, nama: pre.nama })}
                          note={!post ? `Belum ada pasangan Post-Test` : undefined}
                          fullWidth={!post}
                        />
                      ) : (
                        <div />
                      ...
                      {post ? (
                        <BankCard
                          bank={post}
                          active={selectedBankId === post.id}
                          onSelect={() => setSelectedBankId(post.id)}
                          onRename={() => setOpenRename({ id: post.id, nama: post.nama })}
                          onDelete={() => setConfirmDeleteBank({ id: post.id, nama: post.nama })}
                          note={!pre ? `Belum ada pasangan Pre-Test` : undefined}
                          fullWidth={!pre}
                        />
                      ) : null}
                    </div>
        ...
        <section className="bg-white/80 backdrop-blur-sm rounded-3xl border-2 border-gray-100 shadow-xl overflow-hidden">
          <SectionHeader
            title="Daftar Soal"
            subtitle={selectedBankId ? banks.find((b) => b.id === selectedBankId)?.nama || "Pilih salah satu bank soal" : "Pilih salah satu bank soal"}
            actionButton={
              <button
          ...
          <div className="px-6 py-4">
            {!selectedBankId ? (
              <EmptyState icon={ListChecks} title="Pilih Bank Soal" description="Pilih bank soal dahulu untuk melihat pertanyaan" />
            ) : soalLoading ? (
              <LoadingSpinner text="Memuat daftar soal..." />
            ) : (
              <div className="space-y-4">
                {soal.length > 0 && (
                  <div className="mb-2">
                    <SearchInput value={soalQuery} onChange={setSoalQuery} placeholder="Cari di daftar soal..." />
                  </div>
                )}
                {filteredSoal.length === 0 ? (
                  <EmptyState icon={Info} title="Belum Ada Soal" description={soal.length === 0 ? 'Klik tombol "Tambah Soal" untuk membuat pertanyaan pertama' : "Tidak ditemukan hasil pencarian"} />
                ) : (
                  <div className="space-y-4">
                    {filteredSoal.map((q, idx) => (
                      <SoalCard key={q.id} soal={q} index={idx} onDelete={() => setConfirmDeleteSoal(q.id)} />
                    ))}
                  </div>
                )}
              </div>
      ...
      {/* Tambah Bank Modal */}
      {openAddBank && (
        <GreenModalFrame titleIcon={<Plus className="h-6 w-6" />} title="Tambah Bank Soal" subtitle="Buat bank soal baru untuk assessment" onClose={() => setOpenAddBank(false)}>
      ...
      {/* Rename Bank Modal */}
      {openRename && (
        <GreenModalFrame titleIcon={<Pencil className="h-6 w-6" />} title="Ubah Nama Bank Soal" subtitle="Perbarui nama agar lebih jelas" onClose={() => setOpenRename(null)}>
          <div className="p-6 space-y-5">
            <div className="space-y-2">
              <label className="font-semibold text-gray-900 text-sm flex items-center gap-1">
                Nama Bank Soal <span className="text-red-500">*</span>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button onClick={handleRenameBank} className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl py-3 font-semibold transition-all hover:scale-105 shadow-lg">
                Simpan
              </button>
            </div>
          </div>
      ...
      <ConfirmModal
        ...
        description={
          <>
            Apakah Anda yakin ingin menghapus <span className="font-semibold">{confirmDeleteBank?.nama}</span> beserta seluruh soalnya? Tindakan ini tidak dapat dibatalkan.
          </>
        }
        confirmText="Hapus"
      ...
      {/* Tambah Soal */}
      {openAddSoal && (
        <GreenModalFrame titleIcon={<Plus className="h-6 w-6" />} title="Tambah Soal Baru" subtitle="Lengkapi formulir di bawah" onClose={() => setOpenAddSoal(false)} maxWidth="max-w-3xl">
          <form onSubmit={handleCreateSoal} className="p-6 space-y-5 overflow-y-auto max-h-[calc(90vh-100px)]">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={addOption} className="text-sm font-semibold text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
                  <Plus className="h-4 w-4" />
                  Tambah Opsi
                </button>
              </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t-2 border-gray-100">
              ...
              <button type="submit" className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl py-3 font-semibold transition-all hover:scale-105 shadow-lg">
                Simpan Soal
              </button>
            </div>
          </form>
